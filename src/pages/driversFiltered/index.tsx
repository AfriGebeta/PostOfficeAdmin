...
export default function Drivers() {
  ...
  const handleTaskFetch = async () => {
    ...
    console.log(fetchEmployees, "from new friveder======", user?.id, user?.Employee[0].branchId);
    // @ts-ignore    
    fetchEmployees = fetchEmployees.filter(empl => empl.isDriver && empl.branchId === user?.Employee[0].branchId)
